...
### B32【高】boulder 单值事实源：两个会话同根跑 /ulw 互相覆盖指针（2026-09-03 用户提问时发现）

- **现象**：两个会话在同一项目根各跑一次 `/ulw`，第二个会话在第二步写 boulder 后，第一个会话的 `active_goal` /
  `works` / `session_ids` 全部被覆盖；两个 goal 文件反而都活着。次生影响更隐蔽：A 会话此后按第一步「先查 boulder」
  做续跑判定，读到的一直是 **B 的目标**——B18 那条检查反过来误导 A。
- **根因**：`.omz/boulder.json` 是**单值**事实源，`active_goal` / `active_plan` / `active_team` 都是单槽，
  两个会话都想拥有同一个槽位。实测（两会话按协议顺序落盘）：`works` 从 `["A-work"]` 变成 `["B-work"]`，
  `session_ids` 从 `["sess_AAA"]` 变成 `["sess_BBB"]`，A 的记录无痕消失。
- **为什么加锁不对**：`writeJsonSafe` 用 tmp + rename，本来就不可能写出半截文件，这里没有「写交错」可防。
  加锁只是让 B 排队等 A 写完，**排完照样覆盖**——病根是「单槽位两个主人」，不是写的时机。锁是给
  「读-改-写丢更新」用的原语，用在这里是错配。
- **B18 的真实要求是「可发现」，不是「单值」**：新会话需要一个固定的发现入口，不需要那个入口只有一个答案。
  有两个未关闭目标时，正确行为是**列出来让用户选**，而不是只保留一个。单值只是「假设同时只有一个会话」时最省事的编码。
- **解决方案**（1.8.0 已实现，`adapters/zcode/boulder.mjs`）：
  ① 事实源改为 `.omz/boulder/<stem>.json`，**每会话一个槽位文件**，每个会话只写自己那一个；发现机制是
  `readdir` 那个目录——**目录本身就是索引**，没有共享可变索引也就没有丢更新，**全程不需要锁**。
  ② `.omz/boulder.json` 降级为**派生视图**（带 `"source": "derived"` 标记，只喂看板），**永不参与续跑决策**；
  正因为它不参与决策，它输给竞态才无害。保留 render-status 依赖的四个顶层字段，旧看板零改动可读。
  ③ 续跑判定改**三分支**：0 个未关闭 → 全新开始；1 个 → 问续跑还是放弃；**≥2 个 → 列候选让用户选**
  （按 `updated_at` 倒序，逐条给 `stem`/`active_goal`/`updated_at`/`status`）。第三分支顺带修掉一个**单会话下
